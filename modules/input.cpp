...
int isMouseButtonDown(lua_State *L) {
    ...
    lua_pushboolean(L, mouseFlags & SDL_BUTTON_MASK((int) lua_tonumber(L, 1)));
    return 1;
}
...
static const std::unordered_map<std::string, SDL_MouseButtonFlags> mouseButtonCodes = {
...
void registerKeyCodes(lua_State* L) {
    for (auto key : keyCodes) {
        lua_pushstring(L, key.first.c_str());
        lua_pushnumber(L, key.second);
        lua_settable(L, -3);
    }
}

void registerMouseButtonCodes(lua_State* L) {
    for (auto key : mouseButtonCodes) {
        lua_pushstring(L, key.first.c_str());
        lua_pushnumber(L, key.second);
        lua_settable(L, -3);
    }
